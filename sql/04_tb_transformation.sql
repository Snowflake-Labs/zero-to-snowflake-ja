...
Step 1 - Zero Copy Cloning
 ...
 Tasty Bytes Fleet Analysisの一環として、当社の開発者は、Year、Make、Modelを組み合わせた新しいトラックタイプの列を
...
Step 2 - クエリ結果キャッシュの使用
 ...
 テーブルのデータが前回クエリが実行されたときから変更されていない場合、クエリの結果は同じになります。
 クエリを再度実行する代わりに、Snowflakeは結果セットキャッシュから前回と同じ結果を単に返します。
 このステップでは、この機能をテストします。
----------------------------------------------------------------------------------*/

-- Snowflakeの結果キャッシュをテストするために、ウェアハウスを一時停止しましょう。
...
このステップでは、以前に作成した開発用トラックテーブルにトラックタイプのカラムを追加し、更新します。
...
先ほどUpdateステートメントでミスを犯し、YearとMakeの間にスペースを挿入するのを忘れてしまいました。 
幸いにもTime Travel機能を使用して、スペルミスを修正した後のテーブルの状態に戻すことができます。 
これにより、作業を修正することができます。 

Time Travel機能では90日以内であれば、変更または削除されたデータにアクセスすることができます。 
...
このステップでは、開発用トラックテーブルを現在運用中のものと入れ替え本番環境で現在利用可能なものに置き換えます。
----------------------------------------------------------------------------------*/
