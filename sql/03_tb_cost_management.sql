...
Step 1 - 仮想ウェアハウスと設定
 ...
 Snowflakeが提供する機能について理解を深め、
 データから価値を引き出す前に、適切なコスト管理を確立する必要があります。

 このステップでは、最初のSnowflake Warehouseを作成します。これは、仮想コンピュートとというものです。
 
 Snowflakeでは、割り当てられたワークロードに対して可能な限り最小のサイズのウェアハウスから始めることを推奨しています。
 そのため、テスト用ウェアハウスはX-Small で作成します。
----------------------------------------------------------------------------------*/
...
/**
1) ウェアハウスタイプ: 検索やテーブルへのデータのロードを含むすべての DML 操作には、ウェアハウスが必要です。
Snowflake は標準(最も一般的な)または Snowpark 最適化のウェアハウスタイプをサポートしています。
Snowpark 最適化のウェアハウスはメモリ集約型のワークロードに適用すべきです。

2) ウェアハウスサイズ: サイズはウェアハウス内のクラスタごとに利用可能なコンピュートリソースの量を指定します。
Snowflake は、X-Small から 6X-Large までのサイズをサポートしています。

3) 最大クラスタ数:マルチクラスタウェアハウスでは、Snowflakeは静的または動的に追加のクラスタを割り当て
より大規模なコンピュートリソースプールを利用できるようにします。
マルチクラスタウェアハウスは、以下のプロパティを指定することで定義されます。
- 最小クラスタ数:最小クラスタ数、最大数(10)以下。
- 最大クラスタ数:最大クラスタ数、1(最大10)以上。

4) スケーリング・ポリシー:オートスケール・モードで稼働するマルチクラスタ・ウェアハウスにおけるクラスタの自動起動および自動停止のポリシーを指定します。

5) 自動サスペンド:デフォルトでは、自動サスペンドは有効になっています。
Snowflakeは、指定した時間(ここでは60秒)にわたって非アクティブな場合、自動的にウェアハウスを一時停止します。

6) 自動再開:デフォルトでは、自動再開は有効になっています。 
ウェアハウスを必要とするステートメントが送信され、そのウェアハウスがセッションの現在のウェアハウスである場合、Snowflakeは自動的にウェアハウスを再開します。
...
Step 2 - ウェアハウスの再開、一時停止、スケーリング
 ...
 ウェアハウスを作成しました。次にビジネスからのいくつかの質問に回答するためにそれを使用してみましょう。
...
-- Plant Palaceブランドのトラックでは、どのようなメニューを提供していますか?
    --> NOTE: Snowflakeは、ウェアハウスを必要とするステートメントが送信されると、自動的にそのウェアハウスを再開します。
SELECT
    m.menu_type,
    m.truck_brand_name,
    m.menu_item_id,
...
-- Snowflakeの弾力的なスケーラビリティを実証するために、ウェアハウスをスケールアップし、より大規模な集約クエリをいくつか実行してみましょう。
...
SELECT
    ...
    SUM(o.price) AS total_sales
FROM
  analytics.orders_v o
JOIN
  analytics.customer_loyalty_metrics_v clm
  ON o.customer_id = clm.customer_id
GROUP BY
  o.customer_id, name
ORDER BY
...
-- 手動で一時停止に
    --> NOTE: 「無効な状態です。ウェアハウスを一時停止できません」というメッセージが表示された場合、以前に設定した自動一時停止がすでに発生していることを意味します。
