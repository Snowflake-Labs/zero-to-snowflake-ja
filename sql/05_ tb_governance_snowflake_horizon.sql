...
/*----------------------------------------------------------------------------------
作業を開始する前に、Snowflakeアクセス制御フレームワークは以下のものを基にしています。
  • ロールベースのアクセス制御(RBAC):アクセス権限は役割に割り当てられ、順にユーザーに割り当てられます。
  • 任意アクセス制御(DAC):各オブジェクトには所有者がおり、その所有者は  そのオブジェクトへのアクセス権限を付与することができます。

Snowflakeのアクセス制御を理解する上で重要な概念は次のとおりです。
  • セキュアブルオブジェクト:アクセス権が付与できるエンティティ。 アクセスは拒否されます。
    セキュアブルオブジェクトはロール(ユーザとは対照的)によって所有されます
      例:データベース、スキーマ、テーブル、ビュー、ウェアハウス、関数など
  • ロール:権限を付与できるエンティティ。ロールは、順番にユーザーに割り当てられます  
    ユーザーに割り当てられます。 ロールは他のロールにも割り当てることができ、ロール階層が作成されることに注意してください  。
  • 特権:オブジェクトへのアクセス権限の定義されたレベル。複数の異なる特権を  付与されるアクセスの粒度を制御するために使用される場合があります。
  • ユーザー:Snowflakeによって認識されるユーザーID。プログラムであるかに関わらず、

まとめ:
  Snowflakeでは、ロールはセキュアなオブジェクトに対する特権のコンテナです。
  • 特権はロールに付与できる
  • ロールはユーザに付与できる
  • ロールは他のロールに付与することも可能(付与されたロールは付与されたロールの特権を継承する)
  • ユーザーがロールを選択すると、そのロール階層内のすべての特権を継承する 
    階層内のすべての特権を継承します。
----------------------------------------------------------------------------------*/
...
システム定義ロールについて理解したので、それらを活用してテスト用のロールを作成し、
そのロールに初期のSnowflake Horizon Governance機能が適用されるカスタマー・ロイヤリティ・データへのアクセス権限を与えてみましょう。
----------------------------------------------------------------------------------*/
...
1 - MODIFY: ウェアハウスのサイズ変更など、あらゆるプロパティの変更を可能にします。
2 - MONITOR: 現在および過去のウェアハウスで実行されたクエリ、およびその使用統計の表示を可能にします。
3 - OPERATE: ウェアハウスの状態変更(停止、開始、一時停止、再開)を可能にします。 また、現在および過去のウェアハウスで実行されたクエリの表示、および実行中のクエリのキャンセルを可能にします。
4 - USAGE: 仮想ウェアハウスを使用し、その結果として、そのウェアハウス上でクエリを実行することを可能にします。SQLステートメントが送信された際に自動的に再開するように設定されている場合、そのウェアハウスは自動的に再開し、ステートメントを実行します。
5 - ALL: そのウェアハウスに対して、所有権を除くすべての権限を付与します。
**/
...
Step 3 - カラムレベルのセキュリティとタグ付け = タグベースのマスキング
  ...
  最初に展開してテストしたいガバナンス機能は、Snowflake タグベースの動的データマスキングです。
  これにより、テストロールの列にある PII データをマスキングできますが、より特権の大きいロールからはマスキングできません。
  テストロールでは可能ですが、より特権のあるロールでは不可です。
----------------------------------------------------------------------------------*/
...
-- タグが作成されたので、それらを「顧客ロイヤリティ」テーブルの該当する列に割り当てましょう。
...
/**
タグを配置したので、特権ロール以外のすべてのロールのデータをマスクするマスクポリシーを作成することができます。

返されるデータ型が暗黙的にカラムデータ型にキャストされる可能性があるすべてのデータ型に対して、1つのポリシーを作成する必要があります。個々のタグには、データ型ごとに1つのポリシーのみを割り当てることができます。
**/

-- 文字列データ型マスクポリシーを作成する
  --> Note: マスキングポリシーは、CASE文などの標準的な条件ロジックで構成されています。
CREATE OR REPLACE MASKING POLICY governance.tasty_pii_string_mask AS (val STRING) RETURNS STRING ->
    CASE
        -- これらのロールは、マスクされていない値にアクセスできます。
        ...
        -- 列がTASTY_PII:EMAILというタグでタグ付けされている場合
        -- @記号より前の文字列をすべてマスクする 
